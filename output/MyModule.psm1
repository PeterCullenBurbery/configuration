...
function Enable-SSH {
    ...
    while ($true) {
        ...
        Write-Host "🔎 Current State — Name: $($service.Name) | Status: $status | StartType: $start_mode"

        $changed = $false

        if ($start_mode -ne "Auto") {
            Write-Host "⚙️ Setting StartType to 'Automatic'..."
        ...
        if (-not $changed) {
            Write-Host "✅ SSHD is Running and set to Automatic. Done."
            break
        }
...
function Enable-SSHFirewallRule {
    ...
    if (-not $rule_covers_active_profile) {
        Write-Host "🔐 No firewall rule allows SSH on port 22 for profile(s): $active_profiles. Creating '$rule_name'..."
        ...
        try {
            ...
            if ($rule.Enabled -eq 'True' -and $rule.Direction -eq 'Inbound' -and $rule.Action -eq 'Allow') {
                Write-Host "✅ Rule '$rule_name' successfully created and verified."
            } else {
                Write-Warning "⚠️ Rule '$rule_name' was created but doesn't match expected parameters."
            }
        } catch {
            Write-Error "❌ Failed to create or verify rule '$rule_name': $_"
        }
    } else {
        Write-Host "✅ SSH is already allowed on port 22 for profile(s): $active_profiles. No action needed."
    }
}
